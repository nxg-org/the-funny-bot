...
import { getYaw, lookingAt, lookingAtXZ, movingAt, toRadians } from "../util/math";
import { attack } from "../util/customAttack";
 ...
 * SIMPLIFYING DOWN TO 1.8 PVP.
 * NO SHIELDING.
 */
export class Swording extends EventEmitter {
    public ticksUntilAttack: number = 0;
...
export class SwordData extends EventEmitter {
    ...
    private tickOverride: boolean = false;
    private targetBlocking: boolean = false;
    ...
    constructor(public bot: Bot, public options: FullConfig = defaultConfig) {
        ...
        this.bot.on("physicsTick", this.checkForBlock);
    ...
    checkForBlock = async () => {
        ...
        if ((this.target.metadata[8] as any) === 3 && this.target.equipment[1]?.name === "shield") {
            ...
            if (!this.targetBlocking) this.ticksSinceLastSwitch = 0;
            this.targetBlocking = true;
    ...
    async doStrafe() {
        ...
        const diff = getYaw(this.target.position, this.bot.entity.position) - this.target.yaw;
